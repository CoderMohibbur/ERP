<x-app-layout>
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Client Notes</h1>
        <a href="{{ route('client-notes.create') }}"
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-100 dark:bg-gray-700">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200">Added By</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($notes as $note)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-100">{{ $note->client->name ?? '-' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-100">{{ Str::limit($note->note, 60) }}</td>
                        <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-100">{{ $note->creator->name ?? '-' }}</td>
                        <td class="px-6 py-4 text-right space-x-2">
                            <a href="{{ route('client-notes.edit', $note->id) }}"
                               class="text-blue-500 hover:text-blue-700 dark:hover:text-blue-300 font-medium">Edit</a>
                            <form action="{{ route('client-notes.destroy', $note->id) }}" method="POST" class="inline">
                                @csrf @method('DELETE')
