<x-app-layout>
    <div class="max-w-7xl mx-auto p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-white mb-4">➕ Record Disciplinary Action</h2>

        <x-validation-errors class="mb-4" />

        <form method="POST" action="{{ route('employee-disciplinary-actions.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="mb-4 md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4"
                              class="w-full px-4 py-2 border rounded-md dark:bg-gray-700 dark:text-white dark:border-gray-600">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="action_taken" id="action_taken"
                        ...
                        @foreach (['verbal_warning', 'written_warning', 'suspension', 'termination', 'other'] as $type)
                            <option value="{{ $type }}" {{ old('action_taken') == $type ? 'selected' : '' }}>
                ...
                {{-- Severity Level --}}
                <div class="mb-4">
                    <label for="severity_level" class="block mb-1 text-sm text-gray-700 dark:text-gray-300">Severity Level (1-5)</label>
                ...
                {{-- Attachment --}}
                <div class="mb-4 md:col-span-2">
                    <label for="attachment_path" class="block mb-1 text-sm text-gray-700 dark:text-gray-300">Attachment (optional)</label>
                    <input type="file" name="attachment_path" id="attachment_path"
                           class="w-full px-4 py-2 text-gray-700 dark:text-white bg-white dark:bg-gray-700 border rounded-md dark:border-gray-600">
                </div>
            </div>
