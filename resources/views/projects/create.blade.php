        ...
        <form method="POST" action="{{ route('projects.store') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="project_code" id="project_code" value="{{ old('project_code') }}" required
                           class="w-full mt-1 px-4 py-2 border rounded-lg dark:bg-gray-700 dark:text-white dark:border-gray-600">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="actual_cost" id="actual_cost" step="0.01" min="0" value="{{ old('actual_cost') }}"
                           class="w-full mt-1 px-4 py-2 border rounded-lg dark:bg-gray-700 dark:text-white dark:border-gray-600">
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status"
                        ...
                        @foreach(['pending', 'in_progress', 'completed', 'cancelled'] as $state)
                            <option value="{{ $state }}" {{ old('status') === $state ? 'selected' : '' }}>
            ...
            <div class="mt-4">
                ...
                <textarea name="description" id="description" rows="4"
                          class="w-full mt-1 px-4 py-2 border rounded-lg dark:bg-gray-700 dark:text-white dark:border-gray-600">{{ old('description') }}</textarea>
            </div>
