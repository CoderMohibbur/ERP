<x-app-layout>
    <div class="w-full mx-auto p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
        ...
        {{-- Department Drawer --}}
        <x-drawer-form
            id="add-department-drawer"
            title="➕ Add Department"
            action="{{ route('departments.store') }}"
            inputId="new_department_name"
        ...
        {{-- Designation Drawer --}}
        <x-drawer-form
            id="add-designation-drawer"
            title="➕ Add Designation"
            action="{{ route('designations.store') }}"
            inputId="new_designation_name"
            inputLabel="Designation Name"
            cancelHandler="closeDesignationDrawer"
        />
    </div>

    {{-- JavaScript for Drawer Handling --}}
    <script>
