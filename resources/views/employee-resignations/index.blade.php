<x-app-layout>
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Employee Resignations</h1>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-100 dark:bg-gray-700">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200">Resignation Date</th>
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200">Effective Date</th>
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200">Status</th>
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200">Approved By</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($resignations as $resignation)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-100">
                            {{ $resignation->resignation_date->format('Y-m-d') }}
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-100">
                            {{ $resignation->effective_date->format('Y-m-d') }}
                        </td>
                        <td class="px-6 py-4 text-sm capitalize text-gray-800 dark:text-gray-100">
                            {{ $resignation->status }}
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-100">
                            {{ $resignation->approvedBy->name ?? '-' }}
                        </td>
                        <td class="px-6 py-4 text-right space-x-2">
                            <a href="{{ route('employee-resignations.edit', $resignation->id) }}"
                               class="text-blue-500 hover:text-blue-700 dark:hover:text-blue-300 font-medium">Edit</a>
                            <form action="{{ route('employee-resignations.destroy', $resignation->id) }}"
