<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto py-8">
        ...
        <form method="POST" action="{{ route('employee-dependents.update', $employeeDependent) }}">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="employee_id" required
                        ...
                        @foreach ($employees as $id => $name)
                            <option value="{{ $id }}"
                                {{ old('employee_id', $employeeDependent->employee_id) == $id ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $employeeDependent->name) }}" required
                ...
                <div>
                    ...
                    <select name="relation" required
                        ...
                        @foreach(['spouse','child','father','mother','sibling','other'] as $rel)
                            <option value="{{ $rel }}"
                                {{ old('relation', $employeeDependent->relation) == $rel ? 'selected' : '' }}>
                                {{ ucfirst($rel) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="dob"
                           value="{{ old('dob', optional($employeeDependent->dob)->format('Y-m-d')) }}"
                           class="mt-1 w-full px-3 py-2 border rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $employeeDependent->phone) }}"
                           class="mt-1 w-full px-3 py-2 border rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="nid_number" value="{{ old('nid_number', $employeeDependent->nid_number) }}"
                           class="mt-1 w-full px-3 py-2 border rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
                </div>
                ...
                <div class="flex items-center mt-2 space-x-2">
                    <input type="checkbox" name="is_emergency_contact" value="1"
                        {{ old('is_emergency_contact', $employeeDependent->is_emergency_contact) ? 'checked' : '' }}
            ...
            <div class="mt-6">
                ...
                <textarea name="notes" rows="3"
                          class="mt-1 w-full px-3 py-2 border rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
                          placeholder="Optional note">{{ old('notes', $employeeDependent->notes) }}</textarea>
            </div>
            ...
            <div class="mt-6 flex justify-end space-x-4">
                ...
                <button type="submit"
                        class="px-6 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Update Dependent</button>
            </div>
        </form>
    </div>
</x-app-layout>
